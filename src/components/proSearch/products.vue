  ...
  <div class="proSearch_container">
    ...
    <div class="lhq_header">
      ...
      <div  class="clearfix tag-item">
        <p>
          <input type="text" :placeholder="placeholderText" v-model="attrKey" />
          <span @click="search()">查询</span>
        </p>
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        keyWords : "",
        attrKey : "",
        placeholderText : "车型/发动机型号/OE号/编码",
        categoryName : "" ,//分类
    ...
    mounted: function() {
      this.$nextTick(function() {
        ...
        console.log(type)
        this_.showNavImg();
        if(this_.categoryName == ""){
          if(type == 0){ //vin码
            this_.vinCodePros();
          }else if(type == 1){ //按属性
            this_.attrSearch();
          }
        }else if(this_.categoryName == '离合器三件套'){
          this_.placeholderText = '车型/发动机型号/编码';
          this_.getLiheqi();
        }else if(this_.categoryName == '点火线圈'){
          this_.placeholderText = '车型/发动机型号/OE号/编码';
          this_.getDhxq();
        }else if(this_.categoryName == '氧传感器'){
          this_.placeholderText = '车型/发动机型号/OE号/编码';
          this_.getYchuan();
        }
    ...
    methods: {
      ...
      getLiheqi(){
        ...
        this_.$api.get({
          url: this_.$apiUrl.api.GetLiheqi + '?mb001=' + this_.$route.query.mb001 + '&tag=&type=&car=&brand=' + this_.userInfo.dataset[0].mr003,
          params: {},
          success: function (data) {
            console.log(data);
          	//this_.bus.$emit('loading', false);
      ...
      getDhxq(){
        ...
        this_.userInfo = JSON.parse(sessionStorage.getItem("userinfo"));
        this_.$api.get({
          url: this_.$apiUrl.api.GetDhxq + '?mb001=' + this_.$route.query.mb001 + '&tag=&type=&car=&brand=' + this_.userInfo.dataset[0].mr003,
          params: {},
          success: function (data) {
          	this_.bus.$emit('loading', false);
          	this_.isLoad = true;
      ...
      getYchuan(){
        ...
        this_.userInfo = JSON.parse(sessionStorage.getItem("userinfo"));
        this_.$api.get({
          url: this_.$apiUrl.api.GetYchuan + '?mb001=' + this_.$route.query.mb001 + '&tag=&type=&car=&brand=' + this_.userInfo.dataset[0].mr003,
          params: {},
          success: function (data) {
            console.log(data);
          	//this_.bus.$emit('loading', false);
